...
Do not place credentials in prompts. A prompt is written briefly to `request.json` and deleted when the worker accepts the turn; crashes before pickup can leave that request on disk. Events omit thought text and full prompts, but answer text, diagnostics, token usage, working directory, wrapper handles, and Grok session UUIDs remain in the local state directory. Treat both handles as execution context and do not share the state directory across untrusted users.
