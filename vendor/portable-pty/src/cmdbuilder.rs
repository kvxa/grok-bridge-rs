...
pub struct CommandBuilder {
    ...
    pub(crate) job_handle: Option<Arc<OwnedHandle>>,
...
impl PartialEq for CommandBuilder {
    fn eq(&self, other: &Self) -> bool {
        self.args == other.args
            ...
            && self.platform_eq(other)
    }
}

impl CommandBuilder {
    #[cfg(unix)]
    fn platform_eq(&self, other: &Self) -> bool {
    ...
    fn platform_eq(&self, other: &Self) -> bool {
        ...
        self.job_handle.as_ref().map(|h| h.as_raw_handle())
            == other.job_handle.as_ref().map(|h| h.as_raw_handle())
    }
}
...
#[cfg(windows)]
impl CommandBuilder {
    /// Admit the spawned process to this Job Object atomically at CreateProcessW.
    pub fn set_job_handle(&mut self, handle: Arc<OwnedHandle>) {
        self.job_handle = Some(handle);
    }

    pub(crate) fn job_handle(&self) -> Option<Arc<OwnedHandle>> {
        self.job_handle.clone()
    }
}
