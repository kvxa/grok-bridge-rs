...
impl PsuedoCon {
    ...
    pub fn spawn_command(&self, cmd: CommandBuilder) -> anyhow::Result<WinChild> {
        ...
        let mut attrs = ProcThreadAttributeList::with_capacity(if cmd.job_handle().is_some() { 2 } else { 1 })?;
        attrs.set_pty(self.con)?;
        let mut job = cmd.job_handle();
        if let Some(job) = job.as_mut() {
            let mut raw = job.as_raw_handle() as winapi::shared::ntdef::HANDLE;
            attrs.set_job(&mut raw)?;
        }
